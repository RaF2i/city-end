...
const CombinedMarquee = () => {
    ...
    return (
        <div className="py-10 md:py-20 mt-10 md:mt-20 w-full relative overflow-hidden">
            {images.map((imagesUrl, index) => (
                <div key={index} className="flex w-full overflow-hidden">
                    <motion.div
                        initial={{ x: index === 0 ? "0" : "-100%" }}
                        animate={{ x: index === 0 ? "-100%" : "0" }}
                        transition={{ ease: "linear", duration: 20, repeat: Infinity }}
                        className="flex flex-shrink-0 gap-8 md:gap-12 lg:gap-20 py-4 md:py-8 pr-12 md:pr-24"
                    >
                        {imagesUrl.map((url, imgIndex) => (
                            <img key={imgIndex} src={url} className="w-[15vw] md:w-[10vw] lg:w-[8vw]" alt="Logo" />
                        ))}
                    </motion.div>
            ...
            <div className="absolute left-0 top-0 w-32 md:w-64 h-full bg-gradient-to-r from-zinc-800 to-transparent pointer-events-none z-10"></div>
            
            {/* Right gradient shadow */}
            <div className="absolute right-0 top-0 w-32 md:w-64 h-full bg-gradient-to-l from-zinc-800 to-transparent pointer-events-none z-10"></div>
        </div>
    );
};
